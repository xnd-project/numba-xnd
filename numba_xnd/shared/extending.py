...
def create_numba_type(name, llvm_type):
    ...
    numba.extending.register_model(InnerType)(numba.datamodel.models.OpaqueModel)

    return InnerType()


class WrapperType:
    def __init__(self, numba_type, name, datamodel):
        self.name = name
        self.numba_type = numba_type
        self.WrapperNumbaType = self.create_wrapper_numba_type()
        numba.extending.register_model(self.WrapperNumbaType)(datamodel)

        numba.extending.lower_cast(type(numba_type), self.WrapperNumbaType)(
...
class WrappedCStruct(WrapperType):
    def __init__(self, name, attrs, embedded=tuple(), create_wrapper=False):
        ...
        self.NumbaModel = numba.extending.register_model(self.NumbaType)(
            numba.datamodel.models.OpaqueModel
        )
        ...
        if not create_wrapper:
            return
        super().__init__(self.numba_type, name, self.NumbaModel)

    def __str__(self):
        return f"{self.name}(n_bytes={self.n_bytes})"
    ...
    def create_impl(self, typingctx, n_t=numba.types.Const(1)):
        ...
        def codegen(context, builder, sig, args):
            return builder.bitcast(
                numba.cgutils.alloca_once(builder, self.llvm_type, n_t.value),
                self.llvm_ptr_type,
            )
...
def create_tuple_creator(f, n):
    ...
    for i in range(1, n):
        creator = numba.njit(
            lambda args, creator=creator, i=i: creator(args) + (f(i, *args),)
        )
